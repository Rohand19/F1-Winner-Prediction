...
def sample_historical_data():
    ...
    qualifying_data = pd.DataFrame(
        {"DriverId": ["VER", "HAM", "LEC"] * 2, "Position": [1, 2, 3, 2, 1, 3]}
    )

    return {"race": race_data, "qualifying": qualifying_data}
...
def test_engineer_features_with_valid_data(
    feature_engineer, sample_qualifying_data, sample_historical_data
):
...
def test_feature_ranges(feature_engineer, sample_qualifying_data, sample_historical_data):
    features = feature_engineer.engineer_features(
        qualifying_results=sample_qualifying_data,
        historical_data=sample_historical_data,
        track_info={"Length": 5.0, "Laps": 50},
