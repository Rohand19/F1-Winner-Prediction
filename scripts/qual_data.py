...
for _, row in qualifying_processed.iterrows():
    print(
        f"{row['FullName']} ({row['DriverCode']}): {row['QualifyingTime (s)']:.3f}s - Best from {row['Session']}"
    )
...
qualifying_processed[["FullName", "QualifyingTime (s)", "DriverCode"]].to_csv(
    "qualifying_times.csv", index=False
)
