...
def calculate_dynamic_weights(race_number: int) -> Dict[str, float]:
    ...
    if race_number <= 3:  # Early season
        return {"qualifying": 0.40, "recent": 0.35, "historical": 0.15, "form": 0.10}
    elif race_number <= 10:  # Mid season
        return {"qualifying": 0.30, "recent": 0.40, "historical": 0.20, "form": 0.10}
    else:  # Late season
        return {"qualifying": 0.25, "recent": 0.45, "historical": 0.20, "form": 0.10}
...
def model_teammate_dynamics(race_features) -> None:
    ...
    for team in race_features["TeamName"].unique():
        ...
        if len(team_drivers) == 2:
            ...
            qual_diff = abs(
                team_drivers.iloc[0]["QualifyingPerformance"]
