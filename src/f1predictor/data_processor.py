def _engineer_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        features['Qualifying_Consistency'] = data['Qualifying_Consistency']  # Standard deviation of qualifying positions
        features['Race_Consistency'] = data['Race_Consistency']  # Standard deviation of race positions
        
        # Team car performance features
        features['Team_Straight_Speed'] = data['Team_Straight_Speed']  # Team's average straight-line speed
        ...
        for idx, row in enhanced_data.iterrows():
            ...
            if last_races:
                ...
                if driver_id in last_race['results']:
                    enhanced_data.at[idx, 'Last_Race_Position'] = last_race['results'][driver_id]['position']
                    enhanced_data.at[idx, 'Last_Race_Points'] = last_race['results'][driver_id]['points']
            
            # Calculate season points and position
            season_points = sum(race['results'].get(driver_id, {}).get('points', 0) for race in self.historical_races)
            enhanced_data.at[idx, 'Season_Points'] = season_points
            
            # Calculate recent form (average position in last 3 races)
            recent_races = [race for race in self.historical_races[-3:] if driver_id in race['results']]
            if recent_races:
...
def _add_track_data(self, data):
    ...
    if hasattr(self, 'historical_races') and self.historical_races:
        for idx, row in enhanced_data.iterrows():
            ...
            track_races = [race for race in self.historical_races 
...
def _add_reliability_data(self, data):
    ...
    if hasattr(self, 'historical_races') and self.historical_races:
        ...
        for idx, row in enhanced_data.iterrows():
            ...
            recent_races = [race for race in self.historical_races[-5:] if driver_id in race['results']]
            if recent_races:
                dnfs = sum(1 for race in recent_races if race['results'][driver_id].get('status') == 'DNF')
            ...
            for race in self.historical_races[-5:]:
                team_drivers = [d_id for d_id, result in race['results'].items() 
                               if result.get('team') == team_name]
                if team_drivers:
                    team_races.append((race, team_drivers))
            
            if team_races:
                team_dnfs = sum(1 for race, drivers in team_races 
                               for driver in drivers if race['results'][driver].get('status') == 'DNF')
